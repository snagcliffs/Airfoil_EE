...
from Psi_to_q_generator import Psi_to_q_generator
...
def main(args):
    ...
    ##
    gen = Psi_to_q_generator(args.data_path, 
                             args.rank,
                             args.tau, 
                             m_hist = args.m_hist, 
                             stride = args.stride,  
    ...
    ##
    data_params = [args.m_hist,args.rank]
    ...
    for trial in range(args.n_restarts):
        ...
        Models.append(Psi_to_q_net(data_params,net_params,learning_params))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--save_file', default='Psi_q_LSTM', type=str, help='File name for saved results')

    # Network structure
    parser.add_argument('--rank', type=int, default=32, help='Number of POD modes to use')
