...
class P_to_EE_net(tf.keras.Model):
    ...
    def __init__(self, data_params=None, net_params=None, learning_params=None, restart_file=None, restart_dict=None):
        ...
        super(P_to_EE_net, self).__init__()
    ...
    @tf.function
    def compute_loss(self, P_hist, q):

        q_hat = self.network(P_hist)
    ...
    def train_epoch(self, gen):
        ...
        for j in range(train_batches):
            P_hist,q = gen.next_train()
            train_loss(self.train_step(P_hist,q))
            
                    
        val_loss = tf.keras.metrics.Mean()
        for j in range(val_batches):
            P_hist, q = gen.next_val()
            val_loss(self.compute_loss(P_hist,q))
    ...
    def compute_test_loss(self, gen):
        ...
        for j in range(test_batches):
            P_hist, q = gen.next_test()
            test_loss(self.compute_loss(P_hist,q))
    ...
    def predict_full_data(self, gen):
        ...
        for j in range(n_batches):
            ...
            if gen.m_hist == 1: P_hist_batch = P_hist_batch.reshape(P_hist_batch.shape[0], P_hist_batch.shape[2])
