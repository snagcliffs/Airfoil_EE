...
c     History points
      if (mod(istep,nint(uparam(2))).eq.0) call hpts()
...
c     Get velocity perturbation from fixed timestep file
      inlet_timestep = nint(time / uparam(1))
...
c----------------------------------------------------------------------
      ...
      fmt = "(f9.4, TR4, f8.5, TR4, f9.6, TR4, f9.6:)"
