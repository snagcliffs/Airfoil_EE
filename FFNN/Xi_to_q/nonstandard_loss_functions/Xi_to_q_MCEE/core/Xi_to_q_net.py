...
class Xi_to_q_net(tf.keras.Model):
    ...
    def compute_loss(self, Xi, q):
        """
        Clipped exponential loss
        Small values should be predicted as small, but error is tolerated with no penalty
        Large values or predictions should match and are weighted exponentially in their magnitude.
        """
        alpha = 1
        T = 1
        ...
        """
        z = tf.keras.layers.ThresholdedReLU(q, theta=alpha) - \
            tf.keras.layers.ThresholdedReLU(-q, theta=alpha)
        z_hat = tf.keras.layers.ThresholdedReLU(q_hat, theta=alpha) - \
                tf.keras.layers.ThresholdedReLU(-q_hat, theta=alpha)
        """


        z = tf.multiply(q,tf.cast(tf.math.greater(tf.abs(q), tf.zeros(tf.shape(q),dtype=tf.float64)),tf.float64))
        z_hat = tf.multiply(q_hat,tf.cast(tf.math.greater(tf.abs(q_hat), tf.zeros(tf.shape(q_hat),dtype=tf.float64)),tf.float64))
        
        MCEE = tf.reduce_mean((tf.math.exp(z/T) - tf.math.exp(z_hat/T))**2) + \
               tf.reduce_mean((tf.math.exp(-z/T) - tf.math.exp(-z_hat/T))**2)
